...
export default function Login() {
  ...
  const handleSocialLogin = async (provider: string) => {
    try {
      ...
      setError(
        `Error signing in with ${provider}: ${(error as Error).message}`,
      );
    }
  };

  return (
    <div class="w-full max-w-md p-8 mx-auto space-y-8 transition motion-opacity-in-0 motion-translate-y-in-50 motion-duration-600 motion-ease-spring-bouncier">
