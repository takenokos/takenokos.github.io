        ...
        <div>
          <ul class="space-y-4">
            {comments.map((comment: any) => (
              <li
                key={comment.id}
                class="comment-item p-2 bg-white dark:bg-slate-700 rounded flex gap-2"
              >
                <img
                  src={comment.avatar}
                  class="w-12 h-12 rounded-full shadow shadow-indigo-500"
                />
                <div class="flex-1">
                  ...
                  <div
                    class="dark:prose-invert prose prose-sm prose-slate max-w-none"
                    dangerouslySetInnerHTML={{
                      __html: parseMd(comment.commentText),
                    }}
                  ></div>
                </div>
              </li>
          ...
          <form onSubmit={handleSubmit} class="mt-6">
            ...
            <button
              type="submit"
              class="cursor-pointer px-3 py-1 bg-indigo-500 text-slate-50 text-base rounded hover:bg-indigo-600"
            >
              Submit
            </button>
          </form>
